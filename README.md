...
Папка `data/` не включается в репозиторий, так как данные соревнования являются закрытыми. Для воспроизведения решения файлы `train.csv`, `test.csv` и `sample_submission.csv` нужно положить в папку `data/`.
